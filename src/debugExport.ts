...
const leadWorkflowState = (lead: Lead) => {
    ...
    return {
        ...
        selectedExtractionUrl: lead.selectedExtractionUrl ?? '',
        selectedExtractionReason: lead.selectedExtractionReason ?? '',
        selectedExtractionSource: lead.selectedExtractionSource ?? 'none',
        originalUrlClassification: lead.originalUrlClassification ?? lead.sourceUrlClassification ?? lead.websiteExtraction?.sourceUrlClassification ?? 'unknown',
        ...
        shouldReextractOfficialWebsite: lead.shouldReextractOfficialWebsite ?? false,
        extractionBlockedReason: lead.extractionBlockedReason ?? '',
        ...
        platformListingContamination: lead.websiteExtraction?.platformListingContamination ?? false,
        propertyNameMatchScore: lead.websiteExtraction?.propertyNameMatchScore ?? 0,
        expectedPropertySlug: lead.websiteExtraction?.expectedPropertySlug ?? '',
        crossPropertyLinksRejected: lead.websiteExtraction?.crossPropertyLinksRejected ?? [],
...
export function createLeadDebugExport(lead: Lead, context: { diagnostics?: LeadAgentDiagnostic; analysis?: LeadAgentAnalysis } = {}, options: DebugExportOptions = {}) {
    ...
    return withMetadata('lead', {
        ...
        discoveryContact: lead.discoveryContact,
